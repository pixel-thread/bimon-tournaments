...
export function PollCard({ poll, onVote, votingPollId, votingVote, currentPlayerId, onRefetch, onEntryChange, entryPending, isCouponVerifier, isExpanded: controlledExpanded, onToggleExpand }: PollCardProps) {
    ...
    // Use squadsResult as single source of truth once loaded; polls data only while squads API is still loading
    const squadsLoaded = !!squadsResult;
    const squadCount = squadsLoaded ? squadsResult.squadCount : (poll.squadCount ?? 0);
    const confirmedSquads = poll.allowSquads && GAME.squadSize > 1
        ? Math.min(
            squadsLoaded ? squadsResult.confirmedSquadCount : (poll.confirmedSquadCount ?? poll.squadCount ?? 0),
