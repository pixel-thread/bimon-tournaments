 ...
 *  1. WhatsApp vs Discord comparison (plan-style)
                ...
                <ModalContent>
                    <ModalBody className="py-6 px-5 space-y-4">
                        ...
                        <button
                            ...
                            className="text-[11px] text-foreground/30 hover:text-foreground/50 transition-colors cursor-pointer w-full text-center"
                        >
                            Skip for now →
                        </button>
                    </ModalBody>
...
function CompareRow({ ok, needs, children }: { ok?: boolean; needs?: boolean; children: React.ReactNode }) {
    return (
        <div className="flex items-start gap-1.5 text-[10px] leading-tight">
            ...
            <span className={ok ? "text-foreground/70" : needs ? "text-foreground/40" : "text-foreground/40"}>
                {children}
            </span>
        </div>
    );
